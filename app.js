...
async function init(){
  ...
  [A,H,E,W,T,TOUR,EV] = await Promise.all([
    load('./data/analysis.json?v=0.6'), load('./data/holidays.json?v=0.6'),
    load('./data/events.json?v=0.6'), load('./data/weather.json?v=0.6'),
    load('./data/traffic-daily.json?v=0.6'), load('./data/tourism.json?v=0.6'), load('./data/ev-charger.json?v=0.6')
  ]);
...
function renderInsights(){
 ...
 const items=[
  `<strong>Prostorski obseg:</strong> CABLEX meri promet v smeri v mesto Tržič in iz mesta Tržič na merilnem odseku glavne ceste; sedem parkirišč predstavlja okolico mestnega jedra. Rezultatov ne razlagamo kot stanje celotne Občine Tržič.`,
