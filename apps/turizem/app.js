(() => {
  ...
  function renderMarketAnalysis() {
    ...
    $('marketContext').textContent = selectedPeriod === 'all' ? `${summary.period.from}–${summary.period.to}` : `leto ${selectedPeriod}`;
    if (!top.length) {
  ...
  function renderSlovenianAnalysis() {
    ...
    $('sloveniaContext').textContent = selectedPeriod === 'all' ? 'povprečje po letih' : `leto ${selectedPeriod}`;
    if (!valid.length) {
