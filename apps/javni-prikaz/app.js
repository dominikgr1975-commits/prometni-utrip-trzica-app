(() => {
  ...
  const directionTotals = analysis.trafficDirection?.totals || [];
  const directionSpeeds = analysis.trafficSpeed?.directions || [];
  if (directionTotals.length) {
    makeChart('direction','directionChart',{type:'bar',data:{labels:directionTotals.map(x=>x.label),datasets:[{label:'Število vozil',data:directionTotals.map(x=>x.vehicles),backgroundColor:[colors[1],colors[0]],borderRadius:10}]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{beginAtZero:true,ticks:{callback:v=>fmt.format(v)}}},plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>`${fmt.format(c.raw)} vozil`}}}}});
    const into=directionTotals.find(x=>x.direction===1), out=directionTotals.find(x=>x.direction===0);
    const intoS=directionSpeeds.find(x=>x.direction===1), outS=directionSpeeds.find(x=>x.direction===0);
    document.getElementById('directionInsight').innerHTML=`<h3>Tokova sta skoraj uravnotežena</h3><p>V lokacijo je bilo zaznanih <strong>${fmt.format(into?.vehicles||0)}</strong> vozil (${(into?.share||0).toLocaleString('sl-SI')} %), iz lokacije pa <strong>${fmt.format(out?.vehicles||0)}</strong> vozil (${(out?.share||0).toLocaleString('sl-SI')} %).</p><p>Povprečna hitrost je bila V lokacijo <strong>${(intoS?.averageSpeed||0).toLocaleString('sl-SI')} km/h</strong>, IZ lokacije pa <strong>${(outS?.averageSpeed||0).toLocaleString('sl-SI')} km/h</strong>.</p>`;
  }

  const periodSelect = document.getElementById('periodSelect');
  const measuredDailyRows = (Array.isArray(daily) ? daily : [])
    .filter(r => !r.isPartialDay && Number.isFinite(r.totalVehicles) && /^\d{4}-\d{2}-\d{2}$/.test(r.date || ''))
    .sort((a,b) => a.date.localeCompare(b.date));

  // v1.6: dnevni podatki so na voljo za celotno obdobje; ocenjeni dnevi 2024 so že označeni v viru.
  const allDailyRows = measuredDailyRows;
  const availableYears = [...new Set(allDailyRows.map(r => r.date.slice(0,4)))].sort();
  const latestYear = measuredDailyRows.length ? measuredDailyRows[measuredDailyRows.length-1].date.slice(0,4) : (availableYears[availableYears.length - 1] || 'all');
  const periodLabels = {
    '2023':'Leto 2023 (od avgusta – nepopolni podatki)',
    '2024':'Leto 2024 (z interpolacijo manjkajočih podatkov)'
  };
  const periodOptions = [
    {value:'all', label:'Celotno obdobje'},
    ...availableYears.map(year => ({value:year, label:periodLabels[year] || `Leto ${year}`})),
  ...
  function renderTraffic(){
    ...
    const estimateNote = rows.some(r => r.estimated)
      ? ' · vključeni so označeni interpolirani dnevi 2024'
      : '';
    document.getElementById('periodStatus').textContent = rows.length
      ? `${selectedLabel}: ${rows.length} dni (${from.split('-').reverse().join('. ')}–${to.split('-').reverse().join('. ')})${estimateNote}`
