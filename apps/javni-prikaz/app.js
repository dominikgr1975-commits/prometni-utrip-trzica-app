(() => {
  ...
  // Za leti 2023 in 2024 so na voljo tedenski podatki. Za javno primerjavo
  // delavnikov, vikendov in praznikov jih pregledno pretvorimo v ocenjene dnevne
  // vrednosti. Tedenski seštevek in sestava vozil ostaneta ohranjena.
  const weekly = analysis?.trafficWeekly?.records || [];
  const estimatedWeeks = new Set(analysis?.trafficInterpolation?.estimatedWeeks || []);
  const measuredByDay = [1,2,3,4,5,6,0].map(day => {
    const vals = measuredDailyRows.filter(r => new Date(r.date+'T12:00:00').getDay() === day).map(r => r.totalVehicles);
    return vals.length ? vals.reduce((a,b)=>a+b,0)/vals.length : 1;
  });
  const factorSum = measuredByDay.reduce((a,b)=>a+b,0) || 7;
  const dayFactors = measuredByDay.map(v => v/factorSum);
  const syntheticDailyRows = weekly
    .filter(w => ['2023','2024'].includes((w.week || '').slice(0,4)))
    .flatMap(w => {
      const start = new Date(w.week + 'T12:00:00');
      const typeTotals = w.vehicleTypes || {};
      return dayFactors.map((factor, i) => {
        const d = new Date(start); d.setDate(start.getDate()+i);
        const date = d.toISOString().slice(0,10);
        const vehicleTypes = Object.fromEntries(Object.entries(typeTotals).map(([k,v]) => [k, Math.round(v*factor)]));
        return {date, totalVehicles: Math.round(w.totalVehicles*factor), vehicleTypes, isPartialDay:false, estimated:true, interpolated:estimatedWeeks.has(w.week), sourceWeek:w.week};
      });
    });
  const allDailyRows = [...syntheticDailyRows, ...measuredDailyRows].sort((a,b) => a.date.localeCompare(b.date));
  ...
  function renderTraffic(){
    ...
    const estimateNote = rows.some(r => r.estimated)
      ? (periodSelect.value === '2023' ? ' · dnevne primerjave so ocenjene iz tedenskih meritev' : ' · vključene so ocenjene dnevne vrednosti in interpolirani manjkajoči tedni')
      : '';
