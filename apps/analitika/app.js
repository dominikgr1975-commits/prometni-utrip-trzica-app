...
let A, H, E, W, T, TOUR, EV, O, C, UM;
let selectedPulsePeriod = null;

function init(){
  const d = window.PROMETNI_UTRIP_DATA || {};
  [A,H,E,W,T,TOUR,EV,O,C] = [d.ANALYSIS,d.HOLIDAYS,d.EVENTS,d.WEATHER,d.TRAFFIC_DAILY,d.TOURISM,d.EV_CHARGER,d.OFFENSES,d.SPATIAL_CONTEXT];
  if ([A,H,E,W,T,TOUR,EV,O,C].some(v => !v)) {
    const missing = ['ANALYSIS','HOLIDAYS','EVENTS','WEATHER','TRAFFIC_DAILY','TOURISM','EV_CHARGER','OFFENSES','SPATIAL_CONTEXT'].filter(k => !d[k]);
    throw new Error('Manjkajo lokalni podatki: ' + missing.join(', '));
  }
  UM = buildUnifiedModel();
  renderKpis(); renderInputs(); initPulsePeriodControls(); renderUnifiedModel(); bind(); renderSpatialContext(); renderInsights(); renderFacts();
...
function renderUnifiedModel(){
  ...
  pulseExplanation.innerHTML=`<strong>${periodLabel(r.startMonth,r.endMonth)}:</strong> ${pulseLabel(UM.pulse)}. Indeks ${UM.pulse} pomeni približno <strong>${Math.abs(delta)} % ${delta>=0?'več':'manj'} skupne aktivnosti</strong> od običajnega primerljivega obdobja. Utrip Tržiča je sestavljen kazalnik splošne aktivnosti v občini. Upošteva promet, parkiranje, vreme, dogodke, turizem in druge razpoložljive podatke ter pokaže, kako živahno je bilo izbrano obdobje v primerjavi z običajnim stanjem. Višja vrednost pomeni več aktivnosti, nižja pa mirnejše obdobje. Pri komponenti Dogodki en zapis pomeni aktiven koledarski dan dogodka; večdnevni dogodki prispevajo po en zapis za vsak dan. Kazalnik zato ne meri števila različnih prireditev, njihove velikosti ali obiska, delni robni meseci izvoza pa so manj primerljivi s polnimi meseci.`;
  pulseComponents.innerHTML=Object.entries({traffic:'Promet',parking:'Parkiranje',payments:'Parkirnine',events:'Dogodki · aktivni dnevi',tourism:'Turizem',weather:'Vreme'}).map(([k,label])=>`<div><span>${label}</span><strong>${c[k].score??'–'}</strong><small>običajno = 100</small></div>`).join('');
  dataQualityScore.textContent=`${UM.quality} %`; dataQualityExplanation.textContent=`Pokritost ključnih mesečnih podatkov ${nf1.format(UM.completeness)} %. ${UM.estimatedTraffic} od ${UM.totalTraffic} tednov prometa je ocenjenih z interpolacijo. Razširjeni koledar dogodkov izboljša dnevni in mesečni kontekst, vendar zaradi odsotnosti podatka o obisku ni samostojno vključen v odstotek kakovosti.`;
  modelReliabilityScore.textContent=`${UM.reliability} %`; modelReliabilityExplanation.textContent='Ocena velja predvsem za prometno-parkirni model in združuje popolnost, delež dejanskih meritev ter kakovost večfaktorskega modela. Več dogodkov v koledarju ne pomeni samodejno višje statistične zanesljivosti in ne dokazuje prometnega vpliva.';
...
function updateHourly(){
 ...
 hourlyOutput.innerHTML=`<div class="hourly-stat"><span>Povprečje enoletnega profila</span><strong>${nf1.format(p.average)} %</strong><small>${name.replace(', Tržič','')} · ${nf0.format(p.sampleCount)} meritev</small></div><div class="hourly-stat"><span>Najvišji povprečni termin</span><strong>${p.peak.weekdayName}, ${String(p.peak.hour).padStart(2,'0')}:00</strong><small>${nf1.format(p.peak.average)} % zasedenosti</small></div><div class="hourly-stat"><span>Petek 6.00–12.00</span><strong>${nf1.format(fridayAvg)} %</strong><small>${statement}; razlika ${signed(diff,' t')}</small></div><div class="method-note"><strong>Pomembno:</strong> obdobje ${A.parkingHourly.period.from}–${A.parkingHourly.period.to} predstavlja celo leto 3-urnih meritev. Pri Tržnici podatki ne potrjujejo, da je petek dopoldne bolj poln kot druga delovna dopoldneva; preveriti je treba posamezen datum, dogodke, vreme in sezonske razmere.</div>`;
}
...
function dateAnalysis(date){
 ...
 factual+=events.length?`<p><strong>Dogodki:</strong> ${events.map(x=>x.title).join(', ')}.</p>`:`<p>V naloženem izvozu koledarja za obdobje 1. 11. 2023–10. 8. 2026 za ta datum ni evidentiranega dogodka. Odsotnost zapisa ne dokazuje, da dogodka ni bilo.</p>`;
 const interpretation=[]; if(holiday)interpretation.push('praznik navadno zmanjša delovne migracije, vendar lahko poveča izletniški promet'); if(dn==='sobota'||dn==='nedelja')interpretation.push('vikend spremeni namen in čas poti'); if(weather&&weather.rain>1)interpretation.push('dež lahko poveča delež prihodov z avtomobilom, hkrati pa zmanjša obisk zunanjih dejavnosti'); if(events.length)interpretation.push('v koledarju je za ta dan evidentiran dogodek ali aktivni dan večdnevnega dogodka; brez podatka o obisku ni mogoče oceniti njegovega dejanskega prometnega učinka'); if(!interpretation.length)interpretation.push('datum nima prepoznanega posebnega prazničnega, vremenskega ali prireditvenega dejavnika');
 ...
 return factual+confidenceBox(score,'Ocena zanesljivosti temelji na tem, koliko dnevnih virov je za isti datum dejansko na voljo. Evidentiran dogodek izboljša kontekst dneva, ne pomeni pa potrjenega vpliva na promet ali parkiranje. Mesečna zasedenost parkirišč je samo približek, ne dnevna meritev.')+whyBlock(['CABLEX predstavlja vozila v obeh smereh na enem Cesti Ste Marie Aux Mines: v mesto Tržič in iz mesta Tržič.','Zasedenost vseh parkirišč je za izbrani datum mesečno oziroma sezonsko povprečje, dokler ne dobimo dolgoročnega dnevnega ali urnega izvoza.','Dnevnih parkirnin aplikacija ne ocenjuje iz mesečnega prihodka; manjkajoči podatek raje jasno označi.','Koledar dogodkov ne vsebuje podatka o obiskanosti, zato prisotnost dogodka obravnavamo kot možen kontekst, ne kot izmerjen vzrok spremembe.']);
}
...
function renderSpatialContext(){
 const city=C.settlementTrzic, municipality=C.municipality, bus=C.busStops, f=bus.features;
 const ksParts=city.localCommunities.map(x=>`${x.name.replace('Krajevna skupnost ','')}: ${nf0.format(x.permanentRegistrations)}`).join(' · ');
 spatialContextGrid.innerHTML=[
  `<article class="context-card"><span>Naselje Tržič · 2024</span><strong>${nf0.format(city.permanentRegistrations)}</strong><small>stalnih prijav na ${nf0.format(city.addressPointCount)} naslovnih točkah</small><p>${ksParts}. Občinska evidenca se uporablja kot prostorska kontrola, časovni scenarij pa še naprej temelji na SURS.</p></article>`,
  `<article class="context-card"><span>Občina Tržič · 2024</span><strong>${nf0.format(municipality.localCommunityCount)} KS · ${nf0.format(municipality.settlementCount)} naselij</strong><small>${nf0.format(municipality.addressPointCount)} unikatnih naslovnih točk</small><p>Vsota stalnih in začasnih prijav ni število unikatnih oseb. V javni datoteki so zato uporabljeni le agregati.</p></article>`,
  `<article class="context-card"><span>Avtobusna postajališča · 15. 1. 2018</span><strong>${nf0.format(bus.stopLocationCount)} lokacij</strong><small>${nf0.format(bus.applicableDirectionCount)} upoštevanih smernih strani</small><p>Nadstrešek: ${nf0.format(f.shelter.yes)} · pločnik: ${nf0.format(f.sidewalk.yes)} · prehod za pešce: ${nf0.format(f.crossing.yes)} · vozni red: ${nf0.format(f.timetable.yes)}. Gre za zgodovinski, ne trenutni presek.</p></article>`
 ].join('');
}
function renderInsights(){
 ...
 const items=[
  ...
  `<strong>Občinska evidenca naselij:</strong> evidenca za leto 2024 kaže, da naselje Tržič obsega ${nf0.format(C.settlementTrzic.addressPointCount)} naslovnih točk in ${nf0.format(C.settlementTrzic.permanentRegistrations)} stalnih prijav ter sega v KS Tržič – mesto in KS Ravne. Uporabljena je za razumevanje prostorskega obsega, ne kot nadomestilo za SURS časovno serijo.`,
  `<strong>Avtobusna postajališča:</strong> interna evidenca prevoznika z dne 15. 1. 2018 vsebuje ${nf0.format(C.busStops.stopLocationCount)} lokacij. Med ${nf0.format(C.busStops.applicableDirectionCount)} upoštevanimi smernimi stranmi je evidentiranih ${nf0.format(C.busStops.features.shelter.yes)} nadstreškov, ${nf0.format(C.busStops.features.sidewalk.yes)} pločnikov in ${nf0.format(C.busStops.features.crossing.yes)} prehodov za pešce. Ker gre za starejšo evidenco brez koordinat, ne vpliva na izračune in zahteva terensko osvežitev.`,
  ...
  `<strong>Smeri CABLEX:</strong> smer 1 pomeni V lokacijo, smer 0 pa IZ lokacije. V celotnem obdobju je razmerje skoraj uravnoteženo, vendar so povprečne hitrosti v smeri IZ lokacije višje.`,
  ...
  `<strong>Dogodki:</strong> koledar vsebuje ${nf0.format(E.eventCount)} dnevnih zapisov za obdobje ${E.events[0].date.split('-').reverse().join('. ')}–${E.events.at(-1).date.split('-').reverse().join('. ')}. Večdnevni dogodki so razširjeni na posamezne koledarske dni. Komponenta Dogodki v Utripu zato meri relativno število aktivnih dni dogodkov, ne števila različnih dogodkov, njihove velikosti ali obiska.`,
  ...
  `<strong>Utrip Tržiča:</strong> za obdobje ${periodLabel(UM.pulseResult.startMonth,UM.pulseResult.endMonth)} znaša indeks ${UM.pulse}, pri čemer 100 pomeni običajno aktivnost primerljivih koledarskih mesecev. Komponente: promet ${UM.components.traffic.score??'–'}, parkiranje ${UM.components.parking.score??'–'}, parkirnine ${UM.components.payments.score??'–'}, dogodki ${UM.components.events.score??'–'} (aktivni dnevi dogodkov), turizem ${UM.components.tourism.score??'–'} in vreme ${UM.components.weather.score??'–'}. Robni delni meseci koledarskega izvoza lahko komponento dogodkov znižajo, zato je ne razlagamo kot neposredno merilo števila ali uspešnosti prireditev.`,
...
function renderFacts(){
 ...
 const items=[
  `<strong>Naselje Tržič po občinski evidenci 2024:</strong> ${nf0.format(C.settlementTrzic.permanentRegistrations)} stalnih prijav na ${nf0.format(C.settlementTrzic.addressPointCount)} naslovnih točkah; od tega ${nf0.format(C.settlementTrzic.localCommunities[1].permanentRegistrations)} v KS Tržič – mesto in ${nf0.format(C.settlementTrzic.localCommunities[0].permanentRegistrations)} v KS Ravne. Podatek je prostorski presek, ne nova časovna serija.`,
  `<strong>Opremljenost postajališč:</strong> zgodovinska evidenca z dne 15. 1. 2018 kaže nadstrešek na ${nf0.format(C.busStops.features.shelter.yes)} od ${nf0.format(C.busStops.features.shelter.applicableDirections)} upoštevanih smernih strani, pločnik na ${nf0.format(C.busStops.features.sidewalk.yes)} in prehod za pešce na ${nf0.format(C.busStops.features.crossing.yes)}. Brez novega pregleda tega ne predstavljamo kot trenutno stanje.`,
