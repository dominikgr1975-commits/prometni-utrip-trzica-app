...
async function init(){
  A = await fetch('./data/analysis.json?v=0.51').then(r=>{if(!r.ok) throw new Error('analysis.json'); return r.json();});
...
function updatePopulation(){
 ...
 populationOutput.innerHTML=`<p class="answer">Sprememba za <strong>${delta>=0?'+':''}${nf0.format(delta)} prebivalcev mesta Tržič</strong> bi po demografsko prilagojenem scenariju pomenila približno ${nf0.format(r.loPass)}–${nf0.format(r.hiPass)} dodatnih dnevnih prehodov vozil.</p><div class="metric">${nf1.format(r.low)}–${nf1.format(r.high)} %</div><p class="detail">ocenjena zasedenost ${name.toLowerCase()}. Podatek in scenarij veljata samo za naselje oziroma mesto Tržič, ne za celotno občino. Več prebivalcev ne pomeni enakega števila avtomobilov ali vsakodnevnih voženj.</p>${table}<span class="confidence">Nizka zanesljivost · dolgoročni prostorski scenarij</span>${whyBlock(['Model uporablja zgodovino prebivalstva naselja Tržič 2017–2026 in razpon, ne razmerja 1 prebivalec = 1 avtomobil.','Posredno upošteva otroke, starejše, delovno aktivne, skupne vožnje, hojo, javni prevoz in različne potrebe gospodinjstev.','Na voljo je deset letnih vrednosti prebivalstva naselja Tržič; za boljšo umeritev še vedno potrebujemo registrirana vozila, starostno sestavo, dnevne migracije in podrobnejše podatke o namenu poti.'])}`;
}
...
function renderInsights(){
 ...
 const items=[
  ...
  `<strong>Prebivalci mesta Tržič:</strong> podatki 2017–2026 veljajo samo za naselje Tržič. Vpliv ni linearen; aplikacija ga prikazuje kot demografsko prilagojen razpon in jasno navede predpostavke.`,
